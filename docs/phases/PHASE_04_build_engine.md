...
**Goal:** `trackstudio build` turns a project whose components are in manual mode into a valid SZS,
...
versions + impl_version). Stored in `.ts/cache.json` (atomic write). Hit requires identical key and
...
`NodeFinished`, `NodeFailed`, `BuildFinished(summary)`). Per-build log `.ts/logs/build-<ts>.log`,
keep last 20. Updates `.ts/state.json` for generated outputs.
...
and user enabled it) into `.ts/stage/` under its canonical name; folders (posteffect) copied
...
`wszst check` → Issues (parser from S1 findings) → `.ts/issues.json`. Warnings never fail test builds;
`--strict`/release profile fails on errors.
**Acceptance:** parser tests on recorded outputs (clean, warnings, errors); integration test.

### [ ] P4-T07 — `trackstudio build`
`trackstudio build [dir] [--release] [--force] [--only component] [--json-events]`; human-friendly
